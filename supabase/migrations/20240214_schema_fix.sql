...
begin
  ...
  if exists (select from pg_tables where schemaname = 'public' and tablename = 'youtube_searches') then
    drop policy if exists "Users can view own searches" on public.youtube_searches;
    drop policy if exists "Users can create searches" on public.youtube_searches;
  end if;
end $$;
...
drop table if exists public.youtube_searches cascade;
...
create policy "Users can create searches"
  on public.youtube_searches for insert
  with check (auth.uid() = user_id);
...
-- Add indexes
create index youtube_searches_user_id_idx on public.youtube_searches(user_id);
create index youtube_searches_created_at_idx on public.youtube_searches(created_at);
